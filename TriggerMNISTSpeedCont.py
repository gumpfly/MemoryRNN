...
def generate_batch(batch_size,T_Max,TimeSteps,delta_min,NumClasses,MaxClasses):
  ...
  y=Labels
  z=t_error_mat
  return(x,y,z,OnesMat.astype('float32'),t_read_mat.astype('float32'))
...
if(Mode == 'CenterMass' or Mode == 'SlowPoints'):
    ...
    for i in range(10):
        slowpointsH[int(i*NumRep):int((i+1)*NumRep)] = np.repeat(np.reshape(np.load('RNNDynamicsPaper/MNISTAttractor/Init/BlackBoxAnalysis/' + Architechture + '/' + LearningCurr + '_' + str(Instance) + '/'+RegPointsH+'.npy')[i],[1, -1]),NumRep,axis =0)
    ...
    if(Architechture == 'LSTM'):
        ...
        for i in range(10):
            slowpointsC[int(i*NumRep):int((i+1)*NumRep)] =  np.repeat(np.reshape(np.load('RNNDynamicsPaper/MNISTAttractor/Init/BlackBoxAnalysis/' + Architechture + '/' + LearningCurr + '_' + str(Instance) + '/'+RegPointsC+'.npy')[i],[1, -1]),NumRep,axis =0)
...
if(Mode == 'Init'):
    ...
    InitFile = 'RNNDynamicsPaper/MNIST_SpeedReg/Init/' + '10Dig_' + str(LearningCurr) + '_' + Architechture + '_' + Instance
    InitHiddenStep = 'End'
    InitReadOutStep = 'End'
...
SoftMaxLoss = - tf.reduce_mean((t_max-1)*Z*Y*tf.log(prediction+TINY)+Y*tf.log(prediction+TINY))
...
with tf.Session(config=config) as sess:
    ...
    for step in range(1, training_steps+1):
        ...
        _,pred,Rate,Weights,Biases,OutWeights,OutBiases,Speed, SFLoss = sess.run([train_op1,prediction,loss,weights,biases,out_weights,out_biases, SpeedLoss, SoftMaxLoss], feed_dict={X: batch_x, Y: batch_y,Z: batch_z,W:batch_w, LearnRate: learning_rate, T_ReadVec: InputTimes})
    ...
    np.save(SaveFileName+"/Success/TotalHitRate", TotalHitRateList)
    np.save(SaveFileName+"/Success/ZeroHitRate", hitZeroList)
    np.save(SaveFileName+"/Success/DigitsHitRate", hitNotZerolist)
